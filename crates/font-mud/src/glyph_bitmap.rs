...
impl GlyphBitmap {
    pub fn new(scale: f64, range: Range<f64>, angle_threshold: f64, face: &Face, glyph: GlyphId) -> FontResult<Self> {
        let config: MsdfGeneratorConfig = MsdfGeneratorConfig::default();
        let mut shape = face.glyph_shape(glyph).ok_or(FontError::GlyphShape(GlyphShapeError(glyph.clone())))?;
        shape.normalize();
        shape.edge_coloring_simple(angle_threshold, 0);
        let bounds = shape.get_bound();
        let width = (bounds.width() * scale) as usize;
        let height = (bounds.height() * scale) as usize;
        let framing = bounds.autoframe(width as u32, height as u32, range, None)
    ...
    pub fn copy_into_bitmap(&self, bitmap: &mut Bitmap<Rgb<f32>>, offset_width: usize, offset_height: usize, bitmap_width: usize) {
