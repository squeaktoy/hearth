...
impl JsonAssetLoader for MeshLoader {
    ...
    async fn load_asset(&self, _data: Self::Data) -> anyhow::Result<Self::Asset> {
        ...
        let mesh = MeshBuilder::new(vertex_positions.to_vec(), rend3::types::Handedness::Left)
...
pub struct RendererPlugin {
    ...
    pub command_tx: UnboundedSender<Rend3Command>,
}

impl Plugin for RendererPlugin {
    fn build(&mut self, builder: &mut RuntimeBuilder) {
        builder
            .add_asset_loader(MeshLoader(self.renderer.clone()))
            .add_asset_loader(MaterialLoader(self.renderer.clone()))
            .add_asset_loader(TextureLoader(self.renderer.clone()))
            .add_asset_loader(CubeTextureLoader(self.renderer.clone()))
            .add_plugin(RendererService::new(
                self.renderer.clone(),
